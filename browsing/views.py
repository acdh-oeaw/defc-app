            ...
            if obj.province and obj.province.region:
                writer.writerow(
                    [
                        obj.id,
                        obj.name,
                        "; ".join([obj.name for obj in obj.alias_name.all()]),
                        "; ".join([obj.name for obj in obj.alternative_name.all()]),
                        obj.province.name,
                        obj.province.region.name,
                        obj.province.region.country.name,
                        obj.geographical_coordinate_reference_system,
                        obj.coordinate_source,
                        obj.latitude,
                        obj.longitude,
                        obj.elevation,
                        obj.authorityfile_id,
                        obj.topography,
                        obj.description,
                        obj.exact_location,
                        obj.number_of_activity_periods,
                        "; ".join(
                            [
                                obj.author
                                + " "
                                + obj.title
                                + " "
                                + str(obj.publication_year)
                                + " "
                                + str(obj.place)
                                for obj in obj.reference.all()
                            ]
                        ),
                        obj.comment,
                    ]
    ...
    def render_to_response(self, context, **kwargs):
        ...
        writer.writerow(
            [
                ...
                "Grave age groups",
                "Grave sexes",
                "Grave number of female sex",
                "Grave number of male sex",
                "Grave number of not specified sex",
                "Grave manipulations of graves",
                "Description",
                "Reference",
                "Comment"
                # 43 fields
        ...
        for obj in self.get_queryset():
            writer.writerow(
                [
                    ...
                    "; ".join(
                        [str(obj) for obj in obj.settlement_construction_type.all()]
                    ),
                    "; ".join(
                        [str(obj) for obj in obj.settlement_construction_shape.all()]
                    ),
                    "; ".join(
                        [str(obj) for obj in obj.settlement_building_technique.all()]
                    ),
                    "; ".join(
                        [str(obj) for obj in obj.settlement_special_features.all()]
                    ),
                    ...
                    "; ".join(
                        [
                            str(obj)
                            for obj in obj.cave_rockshelters_evidence_of_occupation.all()
                        ]
                    ),
                    ...
                    "; ".join(
                        [str(obj) for obj in obj.cemetery_or_graves_topography.all()]
                    ),
                    "; ".join(
                        [
                            str(obj)
                            for obj in obj.cemetery_or_graves_mortuary_features.all()
                        ]
                    ),
                    obj.grave_number_of_graves,
                    "; ".join([str(obj) for obj in obj.grave_type.all()]),
                    "; ".join(
                        [str(obj) for obj in obj.grave_type_of_human_remains.all()]
                    ),
                    ...
                    obj.grave_number_of_female_sex,
                    obj.grave_number_of_male_sex,
                    obj.grave_number_of_not_specified_sex,
                    "; ".join(
                        [str(obj) for obj in obj.grave_manipulations_of_graves.all()]
                    ),
                    obj.description,
                    "; ".join([str(obj) for obj in obj.reference.all()]),
                    obj.comment,
        ...
        for obj in self.get_queryset():
            writer.writerow(
                [
                    ...
                    "; ".join(
                        [str(obj) for obj in obj.lithics_unretouched_tools.all()]
                    ),
    ...
    def render_to_response(self, context, **kwargs):
        ...
        writer.writerow(
            [
                ...
                "Research type",
                "Institution",
                "Year of activity start year",
                "Year of activity end year",
                "Project name",
                "Project id",
                "Project leader",
                "Special analysis",
